...
func CreatePagination(projectName string) {
	...
	if os.IsNotExist(err) {
		...
		fmt.Fprintf(destination, "	query := db.Model(resultModel)\n\n")
		fmt.Fprintf(destination, "	if paginate.Status != \"\" {\n")
		fmt.Fprintf(destination, "		query = query.Where(\"status = ?\", paginate.Status)\n")
		fmt.Fprintf(destination, "	}\n")
		fmt.Fprintf(destination, "	if paginate.Search != \"\" {\n")
		fmt.Fprintf(destination, "		query = query.Where(\"name LIKE ?\", \"%\"+paginate.Search+\"%\")\n")
		fmt.Fprintf(destination, "	}\n")
		fmt.Fprintf(destination, "	if paginate.StartDate != \"\" && paginate.EndDate != \"\" {\n")
		fmt.Fprintf(destination, "		query = query.Where(\"created_at BETWEEN ? AND ?\", paginate.StartDate, paginate.EndDate)\n")
		fmt.Fprintf(destination, "	}\n")
		fmt.Fprintf(destination, "	if paginate.UserID != \"\" {\n")
		fmt.Fprintf(destination, "		query = query.Where(\"user_id = ?\", paginate.UserID)\n")
		fmt.Fprintf(destination, "	}\n\n")
		fmt.Fprintf(destination, "	query.Count(&total)\n")
		...
		fmt.Fprintf(destination, "	result := query.Limit(paginate.Limit).\n")
